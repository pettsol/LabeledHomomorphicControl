...
{
	// Run joye_libert_keygen() to generate (pk, sk')
	// pk = (y, N)
	// sk = (y, p)
	joye_libert_keygen(N, y, p, msgsize, keysize);
	// Next, choose a random seed K from {0,1}^{k} for a PRF
	// Assume we have initialized HC already	
	//
	// Set label space is equal to plaintext space, i.e., 2^{32}
	// Output sk = (sk', K) and epk = (pk, label space)
	//
	// Message space of labeled HE is similar to message space of Joye-Libert
}
